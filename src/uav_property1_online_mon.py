...
pubmavros_local_position_pose = rospy.Publisher(name = 'mavros/local_position/pose_mon', data_class = PoseStamped, latch = True, queue_size = 1000)
def callbackmavros_local_position_pose(data):
	global ws, ws_lock
	rospy.loginfo('monitor has observed: ' + str(data))
	dict = message_converter.convert_ros_message_to_dictionary(data)
	dict['topic'] = 'mavros/local_position/pose'
...
pubuav_takeoff = rospy.Publisher(name = 'uav/takeoff', data_class = String, latch = True, queue_size = 1000)
def callbackuav_takeoff(data):
	global ws, ws_lock
	rospy.loginfo('monitor has observed: ' + str(data))
	dict = message_converter.convert_ros_message_to_dictionary(data)
	dict['topic'] = 'uav/takeoff'
	dict['time'] = rospy.get_time()
	ws_lock.acquire()
	while dict['time'] in dict_msgs:
		dict['time'] += 0.01
	...
	ws_lock.release()
	rospy.loginfo('event propagated to oracle')
pub_dict = { 'mavros/local_position/pose' : pubmavros_local_position_pose,  'uav/takeoff' : pubuav_takeoff}
msg_dict = { 'mavros/local_position/pose' : "geometry_msgs/PoseStamped",  'uav/takeoff' : "std_msgs/String"}
def monitor():
	global pub_error, pub_verdict
	with open(log, 'w') as log_file:
		log_file.write('')
	rospy.init_node('uav_property1_online_mon', anonymous=True)
	pub_error = rospy.Publisher(name = 'uav_property1_online_mon/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
	pub_verdict = rospy.Publisher(name = 'uav_property1_online_mon/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
	rospy.Subscriber('mavros/local_position/pose', PoseStamped, callbackmavros_local_position_pose)
	rospy.Subscriber('uav/takeoff_mon', String, callbackuav_takeoff)
...
def main(argv):
	...
	log = '/home/robotlab/code/ros/px4_ws/src/rosmondemo/ROSMonitoringConfigs/property1.txt' 
	actions = {
		'mavros/local_position/pose' : ('log', 0), 
		'uav/takeoff' : ('log', 0)
	}
	monitor()
